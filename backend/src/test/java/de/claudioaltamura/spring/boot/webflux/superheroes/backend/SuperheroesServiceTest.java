...
class SuperheroesServiceTest {
  ...
  void getSuperhero() {
    StepVerifier.create(superheroesService.getSuperhero(1L))
        .expectNextMatches(
            superhero -> superhero.getId().equals(1L) && superhero.getName().equals("Hulk"))
        .verifyComplete();
  }

  @Test
  void getSuperheroNotFound() {
    StepVerifier.create(superheroesService.getSuperhero(99L)).expectNextCount(0).verifyComplete();
  }
  ...
  void getSuperheroByName() {
    StepVerifier.create(superheroesService.getSuperheroByName("Spiderman"))
        .expectNextMatches(
            superhero -> superhero.getId().equals(2L) && superhero.getName().equals("Spiderman"))
        .verifyComplete();
  }

  @Test
  void getSuperheroByNameNotFound() {
    StepVerifier.create(superheroesService.getSuperheroByName("Flash")).verifyComplete();
  }
  ...
  void deleteSuperhero() {
    StepVerifier.create(superheroesService.deleteSuperhero(1L)).verifyComplete();

    StepVerifier.create(superheroesService.getSuperheroes()).expectNextCount(1).verifyComplete();
  }

  @Test
  void getSuperheroes() {
    StepVerifier.create(superheroesService.getSuperheroes()).expectNextCount(2).verifyComplete();
  }
